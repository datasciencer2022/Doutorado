...
public class DataSciencer extends HttpServlet {
		...
		if(action.equals("similaridade")) {
	    	...
	    	if (!response.isCommitted()){
				...
				if (typeResult.equals("P1") || typeResult.equals("P4")) {
					...
					registro3 = lerArqTesauro(tesauro);
					
					inclusive1 = lerArqFraseByFrase(logs);
					inclusive2 = lerArqFraseByFrase(palavrasChave);
					exclusive = lerArqFraseByFraseTesauro(tesauro);	
				}
				else if (typeResult.equals("P2")) {
								
					registro1 = lerArq(logs);
					registro2 = lerArq(tesauro);
					registro3 = lerArqTesauro(palavrasChave);
					
					inclusive1 = lerArqFraseByFrase(logs);
					inclusive2 = lerArqFraseByFraseTesauro(tesauro);
					exclusive = lerArqFraseByFrase(palavrasChave);
				}
				else if (typeResult.equals("P3")) {
					registro1 = lerArq(tesauro);
					registro2 = lerArq(palavrasChave);
					registro3 = lerArqTesauro(logs);
					
					inclusive1 = lerArqFraseByFraseTesauro(tesauro);
					inclusive2 = lerArqFraseByFrase(palavrasChave);
				...
				if(!typeResult.equals("P1")) {
			        ...
			        for (String cada: inclusive1) {
			        	if(inclusive2.contains(Normalizer.normalize(cada, Normalizer.Form.NFD).replaceAll("\\p{InCombiningDiacriticalMarks}+", "")
				    		    .toUpperCase())
		        				&& !exclusive.contains(Normalizer.normalize(cada, Normalizer.Form.NFD).replaceAll("\\p{InCombiningDiacriticalMarks}+", "")
		    		    		    .toUpperCase())
		        				){
			        		//System.out.println("tem no inclusive 1 e inclusive2 mas não no exclusive:"+myList.get(cada));
			        ...
			        for (String cada: inclusive1) {
			        	...
			        	if(inclusive2.contains(Normalizer.normalize(cada, Normalizer.Form.NFD).replaceAll("\\p{InCombiningDiacriticalMarks}+", "")
			        		...
			        		//System.out.println("tem no inclusive 1 e inclusive2 e também no exclusive:"+myList.get(cada));
	            ...
	            while (rowIterator.hasNext()) {
	                   ...
	                   if (!first) {
	                       ...
	                       if (finalTemp1.contains(" ")) {
	    						...
	    						for (String p: cadaP) {
	    							if (p.length()>1) {
	    								palavras.add(Normalizer.normalize(p, Normalizer.Form.NFD).replaceAll("\\p{InCombiningDiacriticalMarks}+", "")
	    						    		    .toUpperCase());
	    						...
	    						if (finalTemp1.length()>1) {
	    							palavras.add(Normalizer.normalize(finalTemp1, Normalizer.Form.NFD).replaceAll("\\p{InCombiningDiacriticalMarks}+", "")
	    					    		    .toUpperCase());
	    						}
	    					}
	...
	private List<Arquivo> lerArqTesauro(String arquivo) {
		List<Arquivo> registros = new ArrayList<Arquivo>();
		int cont=0;
		try  
		{  
	...
	private Set<String> lerArqFraseByFraseTesauro(String arquivo) {
		int cont=0;
		Set<String> resposta = new HashSet<String>();
		try  
		{  
	...
	private static Set<String> joinAll(List<Arquivo> registro1, List<Arquivo> registro2, List<Arquivo> registro3) {
        ...
        try {
        	for(Arquivo arq1: registro1) {
    	    	frase1 = null;
    	    	frase1 = arq1.getLinhaToda().toUpperCase();
    	    	if (!(frase1.isEmpty())) {
    		        ...
    		        for(Arquivo arq2: registro2) {
    	                ...
    	                frase2 = null;
    	                frase2 = arq2.getLinhaToda().toUpperCase();
    	                if (!(frase2.isEmpty())) {
    	                	...
    	                	//Systen.out.println("vai comparar frase 1 com frase 2"+arq1.getLinhaToda()+"\t"+arq2.getLinhaToda());
    		                if (frase2.equals(Normalizer.normalize(frase1, Normalizer.Form.NFD).replaceAll("\\p{InCombiningDiacriticalMarks}+", "")
    				    		    .toUpperCase())
    		                		) {
    		            		...
    		            		for (Arquivo arq3: registro3) {
    		            			...
    		            			frase3 = null;
    		                        frase3 = arq3.getLinhaToda().toUpperCase();
    		                        if (!(frase3.isEmpty())) {
    		                        	...
    		                        	//Systen.out.println("vai comparar frase 2 com frase 3"+arq2.getLinhaToda()+"\t"+arq3.getLinhaToda());
    		                        	
    			                        if(frase3.equals(Normalizer.normalize(frase2, Normalizer.Form.NFD).replaceAll("\\p{InCombiningDiacriticalMarks}+", "")
    			    			    		    .toUpperCase())) {
    			                        	//Systen.out.println("É IGUAL");
    			                	   		resposta.add(arq1.getLinhaToda()+"\t"+arq2.getLinhaToda()+"\t"+arq3.getLinhaToda());
    			                            //Systen.out.println("adicionado ao P1(joinAll): "+arq1.getLinhaToda()+"\t"+arq2.getLinhaToda()+"\t"+arq3.getLinhaToda());
    			                	   		encerrar = true;
    			                        }	
    		                        }
	        ...
	        try {
	        	for(Arquivo arq1: registro1) {
		            ...
		            if (!(frase1.isEmpty())) {
		            	...
		            	if (exclusive.contains(Normalizer.normalize(frase1, Normalizer.Form.NFD).replaceAll("\\p{InCombiningDiacriticalMarks}+", "")
	    		    		    .toUpperCase())) {
			            ...
			            for(Arquivo arq2: registro2) {
		                    ...
		                    if (!(frase2.isEmpty())) {
		                    	...
		                    	if (exclusive.contains(Normalizer.normalize(frase2, Normalizer.Form.NFD).replaceAll("\\p{InCombiningDiacriticalMarks}+", "")
	        			    		    .toUpperCase())) {
		                    		//System.out.println("EXCLUIU : "+frase2);
		                    		continue;
		                    	}
	                    	
		                    	if(Normalizer.normalize(frase2, Normalizer.Form.NFD).replaceAll("\\p{InCombiningDiacriticalMarks}+", "")
						    		    .toUpperCase().equals(Normalizer.normalize(frase1, Normalizer.Form.NFD).replaceAll("\\p{InCombiningDiacriticalMarks}+", "")
						    		    .toUpperCase())
			                    		) {
		...
		{  
	        ...
	        try {
	            while (rowIterator.hasNext()) {
                	   ...
                	   while (cellIterator.hasNext()) {
                           ...
                           case 0:
                        	   if (cell.getCellType() == CellType.STRING) {
                            	   ...
                            	   myListTextCompare.add(gerarString(Normalizer.normalize(cell.getStringCellValue(), Normalizer.Form.NFD).replaceAll("\\p{InCombiningDiacriticalMarks}+", "").toUpperCase().trim()));
                            	   //Systen.out.println("adicionou texto: "+cell.getStringCellValue());
                            	   texto = cell.getStringCellValue();                        	   }
                        	   else {
                        		   myListTexto.add(cell.getNumericCellValue()+"");
                            	   myListTextCompare.add(gerarString(Normalizer.normalize(cell.getNumericCellValue()+"", Normalizer.Form.NFD).replaceAll("\\p{InCombiningDiacriticalMarks}+", "").toUpperCase().trim()));
                            	   //Systen.out.println("adicionou texto:"+cell.getNumericCellValue());
                            	   texto = cell.getNumericCellValue()+"";
                        	   }
                        	   break;
                           case 1:
                        	   
                    		   myListQtd.add(cell.getNumericCellValue());
                    		   textoAux = gerarString(texto).split(" ");
                    		   contFreq = textoAux.length;
                    		   myListFreq.add(cell.getNumericCellValue()/contFreq);
	                   ...
	                   cont++;
	                   if(cont > maximumLinhasProj2) break;
	            }
	...
	private String gerarString(String termo) {
		...
		while (continuar) {
			parte = "";
			if (termo.contains("de")) parte = "de";
			if (termo.contains("do")) parte = "do";
			if (termo.contains("da")) parte = "da";
			if (termo.contains("no")) parte = "no";
			if (termo.contains("na")) parte = "na";
			if (termo.contains("em")) parte = "em";
			if (termo.contains(",")) parte = ",";
			if (termo.contains(" ")) parte = " ";
			if (termo.contains("-")) parte = "-";
			if (termo.contains("(")) parte = "(";
			if (termo.contains(")")) parte = ")";
		...
		try {
				...
				if (!first) {
					...
					if(!(perc >= (porcentagem/100))) {
						if (myListQtd.isEmpty()) {
							resultadosRefinamento.add(new String[] {
									last
								});
						}
						else {
							resultadosRefinamento.add(new String[] {
									last,
									new DecimalFormat("0.00").format((freq/cont)),
									soma+""
								});	
						}
			...
			if(perc >= (porcentagem/100)) {
				...
				if (myListQtd.isEmpty()) {
					...
					resultadosRefinamento.add(new String[] {
							last,
							new DecimalFormat("0.00").format((freq==0?lastFreq:freq)/cont),
							(soma==0?lastQtd:soma)+""
						});	
				}
